...
namespace Utils {
    ...
    namespace Graphics {
        ...
        void setPixel(float x, float y, Graphics::Pixel col = FULL, bool line = false) {
            Vector p { x, y };
            std::vector<Transform::Transformation*> reversed { };
            for (int i = Transform::transformStack.size() - 1; i >= 0; i--) {
                reversed.push_back(Transform::transformStack[i]);
            }
            for (Transform::Transformation* pnt : reversed) {
                p = pnt->transform(p);
            }
            x = p.x;
            y = p.y;
